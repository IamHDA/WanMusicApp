...
public interface SubscriptionRepository extends JpaRepository<Subscription, Long> {
    @Query("SELECT s FROM Subscription s WHERE s.subscriber.id = :memberId AND s.active = true")
    ...
    List<Subscription> findAllByActiveTrueAndEndDateBefore(LocalDate currentDate);
}
